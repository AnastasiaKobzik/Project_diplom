  <table class="table table-responsive">
    <thead class='thead-light'>
      <tr>
        <th scope='col'>№</th>
        <th scope='col'>Имя</th>
        <th scope='col'>Номер телефона</th>
        <th scope='col'>Адрес</th>
        <th scope='col'>Дата и время доставки</th>
        <th scope='col'>Способ оплаты</th>
        <th scope='col'>Сумма</th>
        <th scope='col'>Статус заказа</th>
        <th scope='col'>Курьер</th>
      </tr>
    </thead>
<?php
include "../../db/dbConnect.php";
$idOrder = $_GET['id'];
$numbOrd = $_GET['numbOrd'];
echo "
<div class='buttonComeBack mb-3'>
    <button class='comeBack'>
        <svg xmlns='http://www.w3.org/2000/svg' width='40' height='20' fill='currentColor' class='bi bi-arrow-left' viewBox='0 0 16 16'>
          <path fill-rule='evenodd' d='M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8z'/>
        </svg>
        Вернуться
    </button>
</div>";

$query = "SELECT orders.*, users.name FROM orders,users WHERE orders.id_user = users.id_user AND orders.id_order = $idOrder";
$result = mysqli_query($dbLink, $query) or die("Ошибка БД");
if($result){
  $rows = mysqli_num_rows($result); //кол-во строк
  for ($i = 0;$i < $rows; ++$i){
    $row = mysqli_fetch_row($result);
echo"
    <tbody>
      <tr>
        <th scope='row'>$numbOrd</th>
        <td>$row[11]</td>
        <td>$row[6]</td>
        <td>$row[4]</td>
        <td>$row[3]<br>$row[10]</td>
        <td>$row[7]</td>
        <td>$row[5] р</td>";
      if($row[8]==1){
  echo "<td>Заказ оформляется</td>
        <td>-</td>";        
      }elseif($row[8]==2){
  echo "<td>Заказ одобрен</td>
        <td>-</td>";
      }elseif($row[8]==3){
        $queryCourierName = "SELECT couriers.name FROM couriers,courierorders WHERE courierorders.id_order = $row[0] AND courierorders.id_courier=couriers.id_courier";
      $resultCourierName = mysqli_query($dbLink, $queryCourierName) or die("Ошибка БД");
      if ($resultCourierName) {
        $rowCourierName = mysqli_fetch_row($resultCourierName);
  echo "<td>Курьер забрал заказ</td>
        <td>$rowCourierName[0]</td>";
      }

      }elseif($row[8]==4){
        $queryCourierName = "SELECT couriers.name FROM couriers,courierorders WHERE courierorders.id_order = $row[0] AND courierorders.id_courier=couriers.id_courier";
      $resultCourierName = mysqli_query($dbLink, $queryCourierName) or die("Ошибка БД");
      if ($resultCourierName) {
        $rowCourierName = mysqli_fetch_row($resultCourierName);
  echo "<td>Курьер доставил заказ</td>
        <td>$rowCourierName[0]</td>";
      }

      }elseif($row[8]==5){
  echo "<td>Заказ оплачен</td>
        <td>-</td>";
      }
      
  echo "
        
      </tr>    
    </tbody>";
  }
}
echo"
    <thead class='thead-light'>
      <tr>
        <th scope='col'></th>
        <th colspan='2' scope='col'>Название</th>
        <th scope='col'>Форма</th>
        <th colspan='2' scope='col'>Начинка</th>
        <th scope='col'>Вес</th>
        <th scope='col'>Количество</th>
        <th scope='col'>Украшение</th>
      </tr>
    </thead>
    <tbody>";
$queryProduct = "SELECT basket.*, product.name FROM basket,allorders,product WHERE allorders.id_order = $idOrder AND allorders.id_basket=basket.id_basket AND basket.id_product=product.id_product";
$resultProduct = mysqli_query($dbLink, $queryProduct) or die("Ошибка БД");
if($resultProduct){
  $rowsProduct = mysqli_num_rows($resultProduct); //кол-во строк
  for ($i = 0;$i < $rowsProduct; ++$i){
    $rowProduct = mysqli_fetch_row($resultProduct);
    echo "
      <tr>
        <th scope='row'></th>
        <td colspan='2'>$rowProduct[10]</td>";

        if ($rowProduct[5]==null && $rowProduct[6]==null) {
   echo"<td>-</td>
        <td colspan='2'>-</td>
        <td>$rowProduct[3] г</td>";           
        }else{
          $queryFormFill = "SELECT filling.filling, formcake.form FROM filling, formcake WHERE filling.id_filling = $rowProduct[6] AND formcake.id_formCake = $rowProduct[5]";
          $resultFormFill = mysqli_query($dbLink, $queryFormFill) or die("Ошибка БД");
          if($resultFormFill){
            $rowFormFill = mysqli_fetch_row($resultFormFill);
       echo"<td>$rowFormFill[1]</td>
            <td colspan='2'>$rowFormFill[0]</td>
            <td>$rowProduct[3] кг</td>";   
          }
        }

   echo"
        <td>$rowProduct[4] шт</td>";

        if ($rowProduct[8]==null) {
   echo"<td>-</td>";
        }else{
          $queryDecor = "SELECT name_decoration FROM decoration WHERE decoration.id_decoration = $rowProduct[8]";
          $resultDecor = mysqli_query($dbLink, $queryDecor) or die("Ошибка БД");
          if($resultDecor){
            $rowDecor = mysqli_fetch_row($resultDecor);
          echo"<td>$rowDecor[0]</td>";
          }
        }

 echo"</tr>";
     

  }
echo"</tbody>";
     if($row[8]==1){
echo"<tfoot>
      <tr><td colspan='9' style='text-align: right;'><button type='button' class='btn btn-dark acceptOrder' value='$idOrder'>Принять</button></td></tr>
    </tfoot>";      
     }
}
mysqli_close($dbLink);
?>
  </table>



<script type="text/javascript">
/*нажатие на ПРИНЯТЬ ЗАКАЗ*/
$('.acceptOrder').on('click', function(){
  var request = new XMLHttpRequest();
  request.onreadystatechange = function(){
    if((request.readyState==4) && (request.status==200)){    
      $('#modalOk').modal('show');
      $('#modalOk').html(this.response);
    }
  }
  request.open('GET','pages/admin/acceptOrderAdmin.php?id=' + this.value, true);
  request.send(); 
});

/*нажатие на ВЕРНУТЬСЯ*/
$('.comeBack').on('click', function(){
  location.reload(); 
});
</script>
...
echo "<tr class='hoverMore'>
  ...
  echo "<td class='moreOrdUser'><input type='hidden' value='$row[0]' name='moreOrdUser' class='hiddenOrdUser'/></td>
      </tr>";
      $numberOrder++;
    }
  }
...
echo "<tr class='hoverMore'>
  ...
  echo "<td class='moreOrdNoUser'><input type='hidden' value='$row1[0]' name='moreOrdNoUser' class='hiddenOrdNoUser'/></td>
      </tr>";
      $numberOrder++;
    }
  }
  ...
  $('.moreOrdUser').on('click', function(){
    ...
    request.open('GET','pages/admin/outputMoreOrder.php?id=' + id, true);
    request.send();
  });
...
//Подробнее у заказа НЕЗАРЕГ-ГО польз-ля
  $('.moreOrdNoUser').on('click', function(){
    ...
    request.open('GET','pages/admin/outputMoreOrderNoUser.php?id=' + id, true);
    request.send();
  });
  </script>